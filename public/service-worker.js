...
async function networkFirstWithCache(request, cacheName) {
  try {
    ...
    console.log(
      "[ServiceWorker] API network failed, trying cache:",
      request.url
    );
...
async function cleanupApiCache() {
  try {
    ...
    console.log(
      "[ServiceWorker] Cleaning up API cache, entries:",
      requests.length
    );
    ...
    for (const entry of expiredEntries) {
      console.log(
        "[ServiceWorker] Removing expired cache entry:",
        entry.request.url
      );
    ...
    if (remainingEntries.length > API_CACHE_MAX_ENTRIES) {
      const entriesToRemove = remainingEntries.slice(
        0,
        remainingEntries.length - API_CACHE_MAX_ENTRIES
      );
      for (const entry of entriesToRemove) {
        console.log(
          "[ServiceWorker] Removing old cache entry (over limit):",
          entry.request.url
        );
        await cache.delete(entry.request);
      }
